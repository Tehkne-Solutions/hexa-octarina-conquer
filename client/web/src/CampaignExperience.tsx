...
function readCoachSnapshot(root: HTMLElement): BattleCoachSnapshot {
  ...
  return {
    storyActive: Boolean(root.querySelector(".story-scene")),
    objectiveIndex,
    battleOpen: Boolean(root.querySelector(".living-battle-overlay")),
    constructionOpen: Boolean(root.querySelector(".construction-overlay")),
    victory: Boolean(root.querySelector(".outcome-screen.victory")),
    defeat: Boolean(root.querySelector(".outcome-screen.defeat")),
    aiActive: Boolean(aiCurtain),
    aiMessage: aiCurtain?.querySelector("strong")?.textContent?.trim() ?? null,
  };
}
...
function synchronizeCampaignProgress(root: HTMLElement, snapshot: BattleCoachSnapshot): void {
  ...
  updateLivingCampaignProgress({
    status: snapshot.victory ? "victory" : snapshot.defeat ? "defeat" : "active",
    percent,
    completedObjectives: snapshot.victory ? 5 : completedObjectives,
    lastTurn: readTurn(root),
    building,
    rewards: snapshot.victory
...
function updateCoachHighlight(root: HTMLElement, stepId: BattleCoachStepId | null): void {
  ...
  root.querySelectorAll<HTMLElement>(".battle-coach-highlight").forEach((node) => {
    if (node !== target) node.classList.remove("battle-coach-highlight");
  });
  if (target && !target.classList.contains("battle-coach-highlight")) {
    target.classList.add("battle-coach-highlight");
  }
}

function skipStoryFrames(root: HTMLElement): void {
  let advances = 0;
  const advance = () => {
    const button = root.querySelector<HTMLButtonElement>(".story-scene .story-dialogue .living-primary");
    if (!button || advances >= 6) return;
    advances += 1;
    button.click();
...
export function CampaignExperience({ playerName, onBack }: CampaignExperienceProps) {
  ...
  const [dismissedSteps, setDismissedSteps] = useState<Set<BattleCoachStepId>>(
    () => parseStoredCoachSteps(localStorage.getItem(COACH_STEPS_KEY)),
  );
  ...
  useEffect(() => {
    ...
    const synchronize = () => {
      ...
      frame = window.requestAnimationFrame(() => {
        const snapshot = readCoachSnapshot(root);
        synchronizeCampaignProgress(root, snapshot);

        const nextStep = resolveBattleCoachStep(snapshot);
        const visibleStep = coachEnabled && nextStep && !dismissedSteps.has(nextStep) ? nextStep : null;
        setActiveCoachId((current) => current === visibleStep ? current : visibleStep);
        updateCoachHighlight(root, visibleStep);

        const nextAiMessages = appendUniqueAiMessage(aiMessagesRef.current, snapshot.aiMessage);
        aiMessagesRef.current = nextAiMessages;
        if (previousAiActiveRef.current && !snapshot.aiActive && nextAiMessages.length > 0) {
          setAiSummary(nextAiMessages);
          aiMessagesRef.current = [];
          if (summaryTimerRef.current !== null) window.clearTimeout(summaryTimerRef.current);
          summaryTimerRef.current = window.setTimeout(() => setAiSummary(null), 5_000);
        }
    ...
    return () => {
      observer.disconnect();
      window.cancelAnimationFrame(frame);
      updateCoachHighlight(root, null);
    };
  ...
  const dismissCurrentStep = () => {
    ...
    next.add(activeCoachId);
    localStorage.setItem(COACH_STEPS_KEY, serializeCoachSteps(next));
    setDismissedSteps(next);
    setActiveCoachId(null);
  };

  const disableCoach = () => {
    localStorage.setItem(COACH_ENABLED_KEY, "false");
    setCoachEnabled(false);
    setActiveCoachId(null);
  };

  const replayCoach = () => {
    localStorage.setItem(COACH_ENABLED_KEY, "true");
    localStorage.removeItem(COACH_STEPS_KEY);
  ...
  return (
    <div className="campaign-experience" ref={rootRef}>
      ...
      <button
        type="button"
        className="battle-help-button"
        onClick={replayCoach}
        aria-label="Mostrar tutorial contextual novamente"
        title="Mostrar tutorial novamente"
      >
        ?
      </button>

      {activeCoach ? (
        <aside className={`battle-coach-card coach-${activeCoach.id}`} role="dialog" aria-labelledby="battle-coach-title">
          <div className="battle-coach-heading">
            <span aria-hidden="true">✦</span>
            <div><small>{activeCoach.eyebrow}</small><strong id="battle-coach-title">{activeCoach.title}</strong></div>
          </div>
          <p>{activeCoach.description}</p>
          <div className="battle-coach-actions">
            {activeCoach.id === "story" ? (
              <button type="button" className="coach-secondary" onClick={() => rootRef.current && skipStoryFrames(rootRef.current)}>
                Pular introdução
              </button>
            ) : null}
            <button type="button" className="coach-secondary" onClick={disableCoach}>Pular tutorial</button>
            <button type="button" className="coach-primary" onClick={dismissCurrentStep}>{activeCoach.actionLabel}</button>
          </div>
        </aside>
      ) : null}

      {aiSummary ? (
        <aside className="ai-turn-summary" aria-live="polite">
          <div><span aria-hidden="true">◉</span><small>RESUMO DA IA</small><strong>O controle voltou para você</strong></div>
          <ul>{aiSummary.map((message) => <li key={message}>{message}</li>)}</ul>
          <button type="button" onClick={() => setAiSummary(null)} aria-label="Fechar resumo da IA">×</button>
        </aside>
      ) : null}
    </div>
  );
