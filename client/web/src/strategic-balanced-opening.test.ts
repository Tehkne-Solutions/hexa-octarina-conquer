...
describe("META 10.26 balanced gameplay opening", () => {
  it("starts from the validated zero-bias geometry with no prebuilt roads", () => {
    const board = createBalancedStrategicBoard();

    expect(strategicUnit(board, "kael").nodeId).toBe("s-0-0");
    expect(strategicUnit(board, "lyra").nodeId).toBe("s-1-2");
    expect(strategicUnit(board, "brakk").nodeId).toBe("s-2-2");
    expect(strategicUnit(board, "varg").nodeId).toBe("s-1-0");
