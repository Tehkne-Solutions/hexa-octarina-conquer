    ...
    <>
      <defs>
        <linearGradient id={`${id}-armor`} x1="0" x2="1" y1="0" y2="1">
          ...
          <stop offset="0.42" stopColor="#6689be" />
          <stop offset="1" stopColor="#152641" />
        </linearGradient>
        <linearGradient id={`${id}-cape`} x1="0" x2="0" y1="0" y2="1">
          <stop stopColor="#5ba0ff" />
          <stop offset="1" stopColor="#13285f" />
        </linearGradient>
        <radialGradient id={`${id}-core`}>
          <stop stopColor="#ecffff" />
          <stop offset="0.45" stopColor="#79e8ff" />
      ...
      <path d="M44 23 H53" stroke="#84f0ff" strokeWidth="2" strokeLinecap="round" />
      <path d="M34 49 L22 55 L20 79 L31 84 L40 66 Z" fill="#4b6d98" stroke="#dceeff" strokeWidth="2" />
      <path d="M61 48 L70 51 L76 72 L69 77 L57 62 Z" fill="#4b6d98" stroke="#dceeff" strokeWidth="2" />
      <path d="M37 70 L34 104 L43 107 L49 78 Z" fill="#263e65" stroke="#a5c4e7" strokeWidth="2" />
      <path d="M52 76 L54 106 L64 106 L61 70 Z" fill="#263e65" stroke="#a5c4e7" strokeWidth="2" />
      ...
      <circle cx="49" cy="54" r="6" fill={`url(#${id}-core)`} className="sprite-core" />
      <path d="M49 49 V59 M44 54 H54" stroke="#efffff" strokeWidth="1.5" />
    </>
  );
}
...
function ArcherArt({ id }: { id: string }) {
  return (
    <>
      ...
      <circle cx="49" cy="58" r="6" fill={`url(#${id}-core)`} className="sprite-core" />
      <path d="M49 53 L53 58 L49 63 L45 58 Z" fill="#efffff" opacity=".9" />
    </>
  );
}
    ...
    <>
      <defs>
        <linearGradient id={`${id}-iron`} x1="0" x2="1" y1="0" y2="1">
          <stop stopColor={elite ? "#d1c29e" : "#a9b0bb"} />
          <stop offset="0.5" stopColor={elite ? "#665541" : "#4f5866"} />
          <stop offset="1" stopColor="#202630" />
        </linearGradient>
        <linearGradient id={`${id}-cloth`} x1="0" x2="0" y1="0" y2="1">
          <stop stopColor={elite ? "#f27a46" : "#d74f45"} />
      ...
      {elite ? <path d="M37 13 L42 3 L49 12 L56 3 L62 15" fill="#c59242" stroke="#ffe5a0" strokeWidth="2" /> : null}
      <path d="M40 25 L58 25 L55 36 L43 36 Z" fill="#181a1f" />
      <path d="M43 29 H55" stroke="#ff725f" strokeWidth="2.5" strokeLinecap="round" />
      ...
      <path d="M75 33 C70 22 85 17 92 25 C85 35 80 39 75 33 Z" fill={elite ? "#d0b26b" : "#afb4bb"} stroke="#eceff3" strokeWidth="2" />
      <path d="M77 25 L91 32" stroke="#5d646e" strokeWidth="3" />
      <circle cx="50" cy="57" r="6" fill={elite ? "#ffb34c" : "#ff6b4c"} opacity="0.82" className="sprite-core" />
      <path d="M45 57 H55 M50 52 V62" stroke="#fff0d5" strokeWidth="1.5" />
    </>
  );
}
...
export function FantasyUnitSprite({ unit, selected = false, compact = false }: FantasyUnitSpriteProps) {
  ...
  const stateLabel = visualState === "wounded"
    ? "ferido"
  ...
  return (
    <div
      className={`fantasy-unit-sprite faction-${unit.faction} role-${unit.role} state-${visualState} ${compact ? "compact" : ""} ${elite ? "elite" : ""}`}
      data-unit-state={visualState}
    >
      <span className="fantasy-unit-shadow" aria-hidden="true" />
      <span className="fantasy-unit-ring" aria-hidden="true" />
      <span className="fantasy-unit-aura" aria-hidden="true" />
      <svg viewBox="0 0 100 125" role="img" aria-label={`${unit.name}, ${unit.title}, ${stateLabel}`}>
        {unit.role === "guardian" ? <GuardianArt id={safeId} /> : unit.role === "archer" ? <ArcherArt id={safeId} /> : <RaiderArt id={safeId} elite={elite} />}
        <g className="sprite-wound-marks" aria-hidden="true">
          <path d="M30 48 L42 56 L35 64 L49 73" />
          <path d="M65 42 L57 53 L69 60" />
        </g>
      </svg>
