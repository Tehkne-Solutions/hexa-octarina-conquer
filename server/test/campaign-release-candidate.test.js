...
test("release-candidate progression unlocks the campaign sequentially from mission 1 through mission 12", () => {
  ...
  for (let index = 0; index < CAMPAIGN_MISSIONS.length; index += 1) {
    ...
    assert.equal(before.missions[index].unlocked, true);
    if (index + 1 < CAMPAIGN_MISSIONS.length) assert.equal(before.missions[index + 1].unlocked, false);
    ...
    if (index + 1 < CAMPAIGN_MISSIONS.length) {
      assert.equal(unlockedMissionIds(progress).includes(CAMPAIGN_MISSIONS[index + 1].id), true);
    }
  }
...
test("release-candidate final state exposes all missions, 36 stars and the authoritative legend achievement only after 12/12", () => {
  ...
  const finalMerge = mergeCampaignResult(progress, victoryResult(CAMPAIGN_MISSIONS.at(-1), CAMPAIGN_MISSIONS.length - 1), 1_800_000_200_000);
...
test("release-candidate replay is idempotent for duplicate rooms and never degrades mastered missions", () => {
  ...
  progress = mergeCampaignResult(progress, first, 1_800_000_300_000).progress;

  const duplicate = mergeCampaignResult(progress, first, 1_800_000_300_100);
  assert.equal(duplicate.recorded, false);
  assert.deepEqual(duplicate.progress, progress);

  const replay = mergeCampaignResult(progress, victoryResult(mission, 99, {
    roomId: "vs72-replay-c1-m1",
    stars: 1,
    stats: { ...first.stats, turns: first.stats.turns + 20, hp: 4 },
  }), 1_800_000_300_200);
