...
RUN set -eux; \
    ...
    test -f /web/public/assets/runtime/registry/canonical-runtime-aliases.json; \
    cp /web/public/assets/runtime/registry/canonical-runtime-aliases.json /tmp/canonical-runtime-aliases.json; \
    rm -rf /web/public/assets/runtime; \
    mkdir -p /web/public/assets/runtime; \
    unzip -oq "/tmp/${runtime_name}" -d /web/public/assets/runtime; \
    mkdir -p /web/public/assets/runtime/registry; \
    cp /tmp/canonical-runtime-aliases.json /web/public/assets/runtime/registry/canonical-runtime-aliases.json; \
    node -e 'const fs=require("node:fs"); const root="/web/public/assets/runtime"; const required=["runtime-install.json","pack-manifest.json","registry/assets-runtime.json","registry/canonical-runtime-aliases.json"]; for(const file of required){if(!fs.existsSync(root+"/"+file)) throw new Error("PACK99_REQUIRED_FILE_MISSING:"+file)} const install=JSON.parse(fs.readFileSync(root+"/runtime-install.json","utf8")); const manifest=JSON.parse(fs.readFileSync(root+"/pack-manifest.json","utf8")); const registry=JSON.parse(fs.readFileSync(root+"/registry/assets-runtime.json","utf8")); const aliases=JSON.parse(fs.readFileSync(root+"/registry/canonical-runtime-aliases.json","utf8")); const assets=Array.isArray(registry.assets)?registry.assets:[]; const unresolved=Array.isArray(registry.unresolved)?registry.unresolved:[]; const requiredMission=["packages/PACK_07_HERO_ROSTER/guardian/directions/HERO_GUARDIAN_01_IDLE_BASE_SW_01.png","packages/PACK_07_HERO_ROSTER/ranger/directions/HERO_RANGER_01_IDLE_BASE_NE_01.png","packages/PACK_08_BASIC_UNITS/recruit/directions/UNIT_RECRUIT_01_IDLE_BASE_NW_01.png","packages/PACK_09_CHAMPIONS_ADVANCED/berserker/directions/CHAMP_BERSERKER_01_IDLE_BASE_NW_01.png"]; const requiredAliasIds=["HERO_GUARDIAN_01_IDLE_BASE_SW_01","HERO_RANGER_01_IDLE_BASE_NE_01","UNIT_RECRUIT_01_IDLE_BASE_NW_01","CHAMP_BERSERKER_01_IDLE_BASE_NW_01"]; const aliasMap=aliases&&aliases.aliases&&typeof aliases.aliases==="object"?aliases.aliases:{}; if(install.packId!=="HOC_PACK_99_FINAL_RUNTIME"||install.profile!=="full"||install.assetCount!==1037||install.unresolvedReferences!==0||registry.packId!==install.packId||registry.profile!=="full"||registry.assetCount!==1037||assets.length!==1037||unresolved.length!==0||!manifest.version){throw new Error("PACK99_FULL_RUNTIME_INVALID")}; if(aliases.packId!==install.packId){throw new Error("PACK99_RUNTIME_ALIAS_PACK_MISMATCH")}; for(const file of requiredMission){if(!fs.existsSync(root+"/"+file)) throw new Error("PACK99_REQUIRED_MISSION_FILE_MISSING:"+file)} for(const id of requiredAliasIds){const file=aliasMap[id]; if(!file) throw new Error("PACK99_REQUIRED_ALIAS_MISSING:"+id); if(!fs.existsSync(root+"/"+file)) throw new Error("PACK99_ALIAS_TARGET_MISSING:"+id+":"+file)} console.log(`PACK99_PRODUCTION_RUNTIME=full canonical=${assets.length} materialized=${assets.length} mission=${requiredMission.length} aliases=${requiredAliasIds.length}`);'; \
    rm -f "/tmp/${runtime_name}" "/tmp/${checksum_name}" /tmp/canonical-runtime-aliases.json /tmp/pack99-production-release.json

RUN npm run build \
    && test -f /web/dist/hoc2.html \
    && cp /web/dist/hoc2.html /web/dist/index.html \
    && test -f /web/dist/assets/runtime/runtime-install.json \
    && test -f /web/dist/assets/runtime/registry/canonical-runtime-aliases.json \
    && echo "HOC2_DOCKER_ROOT=PASS source=hoc2.html target=index.html" \
    && echo "HOC2_DOCKER_PACK99=PASS" \
    && echo "HOC2_DOCKER_RUNTIME_ALIASES=PASS"
